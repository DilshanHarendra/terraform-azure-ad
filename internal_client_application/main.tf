...
resource "azuread_application" "internal_app" {
  ...
  required_resource_access {
    ...
    resource_access {
      id   = local.permission_scopes[index(local.permission_scopes.*.value,"product.write")].id
      type = "Scope"
    }
  ...
  single_page_application {
    redirect_uris = ["http://localhost:4000/","https://jwt.ms/"]
  }
